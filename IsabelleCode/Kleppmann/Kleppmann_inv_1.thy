...
proof (induction rule: participant_induct)
  ...
  then have "\<exists>proc a ack'. (proc \<noteq> 0 \<and> states proc = Committed a ack') \<longleftrightarrow> (proc \<noteq> 0 \<and> states' proc = Committed a ack')"
    using assms(4) by metis
  moreover have "\<exists>sender rcpt. ((sender, Send rcpt Commit) \<in> msgs) \<longleftrightarrow> ((sender, Send rcpt Commit) \<in> msgs')"
