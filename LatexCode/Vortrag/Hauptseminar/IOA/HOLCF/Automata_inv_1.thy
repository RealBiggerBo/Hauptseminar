...
lemma parts_trans_proj:
  assumes "(s, a, s') ∈ parts_trans P"
  shows "if p ∈ P ∧ ((∃q m. a = Receive q p m) ∨ a = Timeout p ∨ a = Restart p ∨ (∃q m. a = Send p q m))
         then (s p, a, s' p) ∈ part_trans p
         else s' p = s p"
  using assms unfolding parts_trans_def by auto

lemma parts_trans_Send:
  assumes "(pstates, Send sender rcpt msg, pstates') ∈ parts_trans P"
  shows "∀p state1 msgs1 state2 msgs2. pstates p = PState state1 msgs1 ∧ pstates' p = PState state2 msgs2 ⟶ state1 = state2"
proof clarify
  fix p state1 msgs1 state2 msgs2
  assume pre1: "pstates p = PState state1 msgs1"
  assume pre2: "pstates' p = PState state2 msgs2"
  have step: "if p ∈ P ∧ sender = p 
              then (pstates p, Send sender rcpt msg, pstates' p) ∈ part_trans p
              else pstates' p = pstates p"
    using parts_trans_proj[OF assms, of p] by auto
  show "state1 = state2"
  proof (cases "p ∈ P ∧ sender = p")
    case True
    then have "(PState state1 msgs1, Send p rcpt msg, PState state2 msgs2) ∈ part_trans p"
...
lemma parts_trans_step_result:
  assumes "(pstates, a, pstates') ∈ parts_trans P"
    and "pstates rcpt = PState pstate pmsgs"
    and "participant_step rcpt pstate a = (new_state, sent)"
    and "rcpt ∈ P"
    and "a = Receive s rcpt m ∨ a = Timeout rcpt ∨ a = Restart rcpt"
  shows "pstates' rcpt = PState new_state (pmsgs ∪ ((λmsg. (rcpt, msg)) ` sent))"
proof -
  from assms(1,4,5) have "(pstates rcpt, a, pstates' rcpt) ∈ part_trans rcpt"
...
lemma System_Receive_chan_step:
  assumes "(((c, ch), p), Receive sender rcpt msg, ((c', ch'), p')) ∈ trans_of (System t P r)"
  shows "(sender, rcpt, msg) ∈ ch ∧ ch' = ch"
  using assms 
...
lemma invariant1_trans:
  assumes "reachable (System t (UNIV - {0}) r) s"
    and "inv1 s"
    and "(s, a, s') ∈ trans_of (System t (UNIV - {0}) r)"
  shows "inv1 s'"
proof -
  ...
  proof (induction rule: System_trans_inv)
    ...
    next
      ...
      then have "(∃msgs p. pstates p = PState PCommitted msgs) ⟷ (∃msgs p. pstates' p = PState PCommitted msgs)"
        by (metis System_Step.hyps(3) part_state.exhaust parts_trans_Send)
      moreover have "(∃sender rcpt. (sender, rcpt, Commit) ∈ chmsgs) ⟶ (∃sender rcpt. (sender, rcpt, Commit) ∈ chmsgs')"
    ...
    next
      ...
      moreover have receive:"(sender, rcpt, msg) ∈ chmsgs'"
      ...
      proof(cases "rcpt = 0")
        ...
        then have "inv1 s ⟷ inv1 s'"
          using inv1_def chEquiv by (metis decomp_s decomp_s' fst_conv snd_conv)
        then show ?thesis
          using assms(2) decomp_s decomp_s' by simp
      next
        case False
        then have otherPartsEquiv:"∀p ≠ rcpt. pstates p = pstates' p"
        ...
        next
          ...
          proof(cases pstate)
            ...
            proof(cases "∃t'. msg = Prepare t'")
              ...
              then have "new_state = Prepared ∨ new_state = PAborted"
                by (meson prod.inject)
              then have "∀msgs. pstates' rcpt ≠ PState PCommitted msgs"
            ...
            next
              case False
              then have "new_state = pstate ∧ sent = {}"
              ...
              then show ?thesis
                using ‹pstates rcpt ≠ pstates' rcpt› by presburger
            qed
          next
            case Prepared
            then show ?thesis
            proof(cases "msg = Commit")
              case True
              then have "(∃sender rcpt. (sender, rcpt, Commit) ∈ snd (fst s))"
            ...
            next
              ...
              proof(cases "msg = Abort")
                case True
                then have "new_state = PAborted ∧ sent = {(sender, Ack)}"
              ...
              next
                case False
                then have "new_state = pstate ∧ sent = {}"
                  using Prepared Receive local.step ‹msg ≠ Commit› ‹msg ≠ Abort› by(cases msg; auto)
                then have "pstates' rcpt = PState pstate pmsgs"
                  using update_eq by simp
                then have "pstates = pstates'"
                  using otherPartsEquiv stateDecomp by force
...
proof(induction rule: invariantI)
  case (1 s)
  moreover have "starts_of (System t (UNIV - {0}) r) = {((CState (CInitial (t 0) UNIV r) {},{}),λp. PState (PInitial (t p)) {})}"
    unfolding starts_of_def System_def par_def Coordinator_def Channel_def Participants_def by auto
  ultimately have "s = ((CState (CInitial (t 0) UNIV r) {},{}),λp. PState (PInitial (t p)) {})"
    by auto
  then show ?case 
